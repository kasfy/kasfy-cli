...
const fs = require('fs');
  ...
  .action((ctrlname) => {
    ...
    var count = ctrlname.length - 10;
    var res = ctrlname.substr(count, 10);
    if (res == 'Controller' || res == 'Controller') {
      var ctrlnewName = ctrlname;
    }
    else {
      var ctrlnewName = ctrlname+'Controller';
    }

    if (fs.existsSync('app/controllers/'+ctrlnewName+'.js')) {
      console.log("\n ❗️ A controller already exists on same name \n");
      process.exit(1);
    }

    fs.writeFile('app/controllers/'+ ctrlnewName +'.js',
